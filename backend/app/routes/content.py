...
def get_content(
    ...
    if intent and intent != "All":
        query = query.filter(Sentiment.intent == intent)
    if date_from:
    ...
    for content, sentiment_row, platform_row in rows:
        results.append(ContentWithSentiment(
            ...
            intent_confidence = sentiment_row.intent_confidence,
        ))
...
def get_metrics(
    ...
    intent_counts = (
        db.query(Sentiment.intent, func.count(Sentiment.id))
        .join(Content, Content.id == Sentiment.content_id)
        .filter(Content.brand_id == brand_id)
        .group_by(Sentiment.intent)
        .all()
    )

        ...
        <div className="flex items-center justify-between h-16">
          ...
          <nav className="hidden md:flex items-center gap-1">
            {NAV_LINKS.map(link => (
              <Link
                ...
                } ${link.path === '/admin' ? 'text-purple-600 hover:bg-purple-50' : ''}`}
              >
                {link.path === '/admin' ? '⚙ Admin' : link.label}
              </Link>
            ))}
          </nav>
          ...
          <div className="hidden md:flex items-center gap-3">
            <div className="flex items-center gap-2.5 px-3 py-1.5 rounded-xl bg-slate-50 border border-slate-200">
              <div className={`w-7 h-7 rounded-lg flex items-center justify-center text-white text-xs font-bold ${isAdmin ? 'bg-purple-500' : 'bg-gradient-to-br from-brand-400 to-brand-600'}`}>
                {initials}
              </div>
              <div>
                <span className="text-sm font-medium text-slate-700">{user?.email?.split('@')[0]}</span>
                {isAdmin && <span className="ml-1.5 text-xs text-purple-500 font-semibold">Admin</span>}
              </div>
            </div>
            <button
          ...
          <div className="md:hidden pb-4 space-y-1 animate-fade-in">
            {NAV_LINKS.map(link => (
              <Link key={link.path} to={link.path} onClick={() => setMobileOpen(false)}
                className={`block px-4 py-2.5 rounded-lg text-sm font-medium transition-all ${location.pathname === link.path ? 'bg-brand-50 text-brand-600' : 'text-slate-600 hover:bg-slate-100'}`}>
                {link.path === '/admin' ? '⚙ Admin' : link.label}
              </Link>
            ))}
            <button onClick={handleLogout} className="w-full text-left px-4 py-2.5 rounded-lg text-sm font-medium text-red-500 hover:bg-red-50">
              Logout
            </button>
          </div>
        )}
